...
final class TutorHostController: ObservableObject {
    ...
    private func handle(_ request: TutorRequest) async -> TutorResponse {
        ...
        do {
            ...
            return failure(request, failure.code, failure.message)
        } catch let failure as DescriptorValidationError {
            return failure(request, "invalid_descriptor", failure.message)
        } catch {
    ...
    private func focusedWindowID(processID: pid_t, frame: CGRect) throws -> CGWindowID {
        ...
        let candidate = infos.first { info in
            guard (info[kCGWindowOwnerPID as String] as? NSNumber)?.int32Value == processID,
                  ...
                  let windowFrame = CGRect(dictionaryRepresentation: bounds) else { return false }
...
private func frameAttribute(_ element: AXUIElement) throws -> CGRect {
    ...
    guard AXUIElementCopyAttributeValue(element, kAXFrameAttribute as CFString, &value) == .success,
        ...
        throw TutorHostFailure(code: "missing_frame", message: "The resolved Accessibility element has no usable local frame")
    }
